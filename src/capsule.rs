...
impl Capsule {
    ...
    pub fn new(position: (f32, f32), arm: (f32, f32), radius: f32) -> Capsule {
        return Capsule {
            ...
            arm,
            radius,
            perp: Capsule::set_perp(arm, radius),
        };
    }
...
impl crate::Shape for Capsule {
    ...
    fn get_axis(&self, index: usize, target: (f32, f32)) -> (f32, f32) {
        return match index {
            ...
            1 => self.perp,
            2 => (
                target.0 - self.position.0 - self.arm.0,
                target.1 - self.position.1 - self.arm.1,
            ),
            _ => (
                target.0 - self.position.0 + self.arm.0,
                target.1 - self.position.1 + self.arm.1,
    ...
    fn project(&self, axis: (f32, f32), normalize: bool) -> (f32, f32) {
        let circle1 = Circle::new(
            (self.position.0 + self.arm.0, self.position.1 + self.arm.1),
            self.radius,
        );
        ...
        return (
            f32::min(projection1.0, projection2.0),
            f32::max(projection1.1, projection2.1),
        );
    }
    ...
    fn point(&self, index: usize) -> (f32, f32) {
        return match index {
            0 => (
                self.position.0 + self.arm.0 + self.perp.0,
                self.position.1 + self.arm.1 + self.perp.1,
            ),
            1 => (
                self.position.0 + self.arm.0 + self.perp.0,
                self.position.1 + self.arm.1 + self.perp.1,
...
mod capsule_tests {
    ...
    fn test_serde() {
        let capsule: Capsule =
            ron::from_str("Capsule(position: (0.0, 0.0), arm: (10.0, 0.0), radius: 5.0)").unwrap();
